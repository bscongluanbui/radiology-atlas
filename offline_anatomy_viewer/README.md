...
- **Scroll** is the default mode: the mouse wheel changes slices. After the first frame opens, the viewer preloads the complete active series into its versioned HTTP cache with two low-priority workers. All slice JSON for that series remains in the in-memory cache, while a larger bounded LRU window of 32 decoded images (20 ahead and 11 behind; at most 16 on clients reporting ≤4 GB RAM) avoids retaining hundreds of full RGBA frames. A fast wheel burst is coalesced to the latest requested position. The previous diagnostic frame stays painted while an uncached slice loads; the full-screen spinner is reserved for initial series loading. Opening another series cancels the old queue and replaces these caches with the new series.
  The web gateway uses a shared 512 MiB/tab Blob + memoized data URL cache for main, filmstrip, MPR and overlays instead of persistent HTTP caching. Image/JSON queues reserve foreground capacity; runtime settings come from Docker environment variables. See [performance settings](../docker/PERFORMANCE.md).
